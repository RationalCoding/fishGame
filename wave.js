...
function run() {
    ...
    function mouseMove(e) {
        ...
        msX = e.clientX;
        msY = e.clientY;
        if (canvas.height - columns[i][0] > msY) {
            return;
        }
    ...
    function draw() {
        ...
        if (textIndex == 0) {
            ctx.fillText("Welcome to Grey Scales", canvas.width / 2, canvas.height * 0.28);
        }
        if (textIndex == 1) {
            ctx.fillText("A simulation game by Thomas Mullen", canvas.width / 2, canvas.height * 0.28);
        }
        if (textIndex == 2) {
            ctx.fillText("The water is disturbed by mouse movement", canvas.width / 2, canvas.height * 0.28);
        }
        if (textIndex == 3) {
            ctx.fillText("Click and hold to drop objects", canvas.width / 2, canvas.height * 0.28);
        }
        if (textIndex == 4) {
            ctx.fillText("The buttons at the top will change what you drop", canvas.width / 2, canvas.height * 0.28);
        }
        if (textIndex == 5) {
            ctx.fillText("There are many secrets to discover...", canvas.width / 2, canvas.height * 0.28);
        }
        if (textIndex == 6) {
            ctx.fillText("Enjoy!", canvas.width / 2, canvas.height * 0.28);
        }
        ...
        if (toolTip != "") {
            ...
            if (toolTip == "Thanks for visiting my website!") {
    ...
    b1.addEventListener('click', function () {
        selected = "fish";
        showToolTip("Tiny fish that need care");
    });
    b2.addEventListener('click', function () {
        selected = "food";
        showToolTip("Food for your fish");
    });
    b3.addEventListener('click', function () {
        selected = "barrels";
        showToolTip("Buoys that bob on the seas");
    });
    b4.addEventListener('click', function () {
        selected = "shark";
        showToolTip("Aggresive fish");
    });
    b5.addEventListener('click', function () {
        selected = "player";
        showToolTip("Arrow+Shift keys for blue, WASD+Space for red");
    });
    github.addEventListener('click', function () {
        clickedGithub = true;
        showToolTip("Thanks for visiting my website!")
    });
